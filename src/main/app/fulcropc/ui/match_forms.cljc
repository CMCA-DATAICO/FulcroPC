(ns app.fulcropc.ui.match-forms
  ...
  (:require
    ...
    [app.fulcropc.model.match :as r.match]
...
#_(form/defsc-form MatchForm [this props]
    {fo/id            r.match/id
     fo/attributes    [r.match/league r.match/date r.match/match-day
                       r.match/local r.match/local-goals
                       r.match/visitor r.match/visitor-goals]
...
(report/defsc-report MatchList [this {:ui/keys [current-rows current-page page-count] :as props}]
  {ro/title             "All Matches"
   ...
   ro/column-formatters {:match/league  (fn [_ v _ _] (str (:league/year v)))
                         :match/local   (fn [_ v _ _] (str (:team/title v)))
                         :match/visitor (fn [_ v _ _] (str (:team/title v)))}
   ro/run-on-mount?     true
   ro/route             "matches"})

#_(report/defsc-report MatchList [this {:ui/keys [current-rows current-page page-count] :as props}]
    {ro/title               "All Matches"
     ro/source-attribute    :match/all-matches
     ro/row-pk              r.match/id
     ro/columns             [r.match/league r.match/match-day r.match/date
     ...
     ro/column-formatters   {:match/league  (fn [_ v _ _] (str (:league/year v)))
                             :match/local   (fn [_ v _ _] (str (:team/title v)))
                             :match/visitor (fn [_ v _ _] (str (:team/title v)))}
     ro/form-links          {r.match/local   TeamForm
                             r.match/visitor TeamForm}
     ro/run-on-mount?       true
     ...
     ro/control-layout      {:action-buttons [::new-match]
                             :input          [:_]}
     ro/initial-sort-params {:sort-by          :match/match-day
                             :ascending?       true
